...
shadcn-web-components uses CSS custom properties (CSS variables) for theming. Load the global `tokens.css` file described in **Guides / Getting Started**, then override its values to match your application's design system.
...
### Method 3: Load Defaults and Override

Load the published global tokens, then override them in your own stylesheet:
...
The package's `/styles` JavaScript subpath exports Lit `CSSResult` helpers. It is
not a global CSS file and cannot be loaded with CSS `@import` or `<link>`.
...
`--transition-base` and `--transition-normal` are equivalent aliases used by
...
These tokens control spinner rotation, skeleton pulsing, and the input OTP caret.
Their components disable keyframe animation when `prefers-reduced-motion: reduce`
is active.
